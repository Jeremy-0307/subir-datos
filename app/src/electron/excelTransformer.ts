...
let columnType = new Map<string, string>();
...
export function excelToTS(filePath: string) {
...
function ColumnHandler(key: string, value: any): any {
        ...
        if (typeof value === 'string') {
          try {
            ...
            const cleanedObject = Object.fromEntries(
              Object.entries(parsed).map(([k, v]) => [k.replace(/"/g, ''), v])
            );
